...
export class IsolateOtherRotationNormalizeCube extends QScene {
    ...
    constructor() {
        ...
        // this.vector(this.origin, axis.scale(1), interpolateColor('var(--purple)', 'var(--background)', 0.25));
        this.vector(this.origin, axis.scale(1), 'var(--purple)');
        ...
        let w = new Quaternion(0, 1, 1, 1).normalize();
        ...
        this.reset = () => {
            ...
            if (singleton) {
                // this.play([
                //     cubeRotation.animate.slerp(b.multiply(a)),
                // ], 5)
                this.play([
                    cube.animate.setOpacity(1),
                ], 5, "linear")
                singleton = false;
            }
    ...
    cube(r: Quaternion = Quaternion.identity(), color: string = 'var(--purple)') {
    ...
    cube2(r: Quaternion = Quaternion.identity(), drawTopFace = false, color: string = 'var(--font-color)') {
        ...
        const projectedVertices = cubeVertices.map(q => {
            ...
            q.update = () => {
                ...
                let a = r.multiply(q_copy).multiply(r.inverse());
                // Apply visualized
                q.set(this.left.multiply(a).multiply(this.right));
            }
            q.update();
            return q;
        });
