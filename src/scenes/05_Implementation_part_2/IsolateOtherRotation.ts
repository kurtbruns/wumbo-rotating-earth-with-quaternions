...
export class IsolateOtherRotation extends QScene {
    ...
    constructor(config: IsolateOtherRotationConfig = {}) {
        ...
        super({
            ...
            cameraPosition: new Vector3(0, 0, -3.5),
            drawAxes: false,
            labelAxes: false,
        ...
        let a = Quaternion.fromAxisAngle(new Vector3(1, 0, 0), Math.PI/4);
        
        let b = Quaternion.fromAxisAngle(new Vector3(-1, 1, 0), Math.acos(1/Math.sqrt(3)));
        

        // let b = Quaternion.orientationBetween(new Vector3(1, 1, 1).normalize(), new Vector3(0, 0, 1));

        let s = new Quaternion(0, 0, 0, 1)
        s = b.multiply(s).multiply(b.inverse());
        console.log(s.toFormattedString());

        this.vectorQ(s, 'var(--pink)', 0.5);
        ...
        // this.drawHypercube();
        // this.axisQuat = Quaternion.orientationBetween(new Vector3(0, 0, 1), new Vector3(1, 1, 1).normalize());
        // this.axisQuat = Quaternion.fromVector(new Vector3(1, 1, 1).normalize());
    ...
    cube(r: Quaternion = Quaternion.identity()) {
        ...
        const projectedVertices = cubeVertices.map(q => {
            ...
            // let b = Quaternion.fromAxisAngle(new Vector3(-1, 1, 0), Math.acos(1/Math.sqrt(3)));
            // q = a.multiply(q).multiply(a.inverse());
            // q = b.multiply(q).multiply(b.inverse());
